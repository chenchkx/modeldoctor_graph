...
class GCN(nn.Module):
    ...
    def forward(self, graphs, nfeat, efeat, channel_mask=None):
        ...
        if not channel_mask is None:
            channel_mask = channel_mask.repeat(g_n.shape[0],1)
            g_n = g_n * channel_mask.float()

...
from optims.modeldoctor_optim_auc_with_cls_ver2 import ModelDoctorOptLearning_CLS
from models.GIN import GIN
from models.GCN_v2 import GCN
...
def ModelOptLoading(model, optimizer, 
    ...
    if args.loss_type == 'cls': 
        modelOptm = ModelDoctorOptLearning_CLS(
                                model=model, 
                                optimizer=optimizer,
...
def args_(args, dataset): 
    ...
    # doctor logs dir
    args.doctor_identity = (f'{args.identity}-{args.runs}')
    args.doctor_logs_dir = 'logs_doctor'
    if not os.path.exists(args.doctor_logs_dir):
        os.mkdir(args.doctor_logs_dir)
    args.doctor_xlsx_dir = os.path.join(args.doctor_logs_dir, 'xlsx')
    args.doctor_imgs_dir = os.path.join(args.doctor_logs_dir, 'imgs')
    args.doctor_dict_dir = os.path.join(args.doctor_logs_dir, 'dict')
    args.doctor_best_dir = os.path.join(args.doctor_logs_dir, 'best')
    return args

def print_best_log(args, key_metric='valid-rocauc', eopch_slice=0, 
                   metric_list='all'):
    logs_table = pd.read_excel(os.path.join(args.doctor_xlsx_dir, args.doctor_identity+'.xlsx'))
    metric_log = logs_table[key_metric]
    best_epoch = metric_log[eopch_slice:].idxmax()
    best_frame = logs_table.loc[best_epoch]
    if not os.path.exists(args.doctor_best_dir):
        os.mkdir((args.doctor_best_dir))
    best_frame.to_excel(os.path.join(args.doctor_best_dir, args.doctor_identity+'.xlsx'))
...
def plot_logs(args, metric_list='all'):
    if not os.path.exists(args.doctor_imgs_dir):
        os.mkdir(args.doctor_imgs_dir)
    logs_table = pd.read_excel(os.path.join(args.doctor_xlsx_dir, args.doctor_identity+'.xlsx'))

    for metric in metric_list:
        metric_epochs = logs_table[metric]
        plt.plot(range(len(metric_epochs)), metric_epochs, 'g-')
        plt.savefig(os.path.join(args.doctor_imgs_dir, args.doctor_identity + f'-{metric}.png'))
        plt.close()
    return 0 
